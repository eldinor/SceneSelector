import { type Scene, Effect, MeshBuilder, PostProcess, Texture, Vector2, Vector3 } from "@babylonjs/core";
...
export function createTestScene2(scene: Scene): void {
  ...
  // Based on "The Drive Home" by Martijn Steinrucken aka BigWings - 2017
  // License Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License.

  // Register the shader code with all effects enabled
  Effect.ShadersStore["driveHomePostProcessFragmentShader"] = `
...
void main(void) {
    ...
    uv.y += bumps*4.;
    CameraSetup(uv, pos, lookat, 2., mousePos.x);
...
`;
  ...
  scene.onPointerMove = (evt) => {
    if (evt.x && evt.y) {
      mousePos.x = evt.x / scene.getEngine().getRenderWidth();
      mousePos.y = 1.0 - evt.y / scene.getEngine().getRenderHeight();
    }
  };
  ...
  box.onDisposeObservable.addOnce(() => {
    postProcess.dispose();
  });
  //
}
